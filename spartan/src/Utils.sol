...
library Utils {
    ...
    function str_to_uint(string memory s) internal pure returns (uint256 res) {
    ...
    function flatten_padded_be_bytes_array(bytes[] memory b) internal pure returns (bytes memory) {
    ...
    //
    // @notice this function will both flat and remove the padding.
    function flatten_padded_le_bytes_array(bytes[] memory b) internal pure returns (bytes memory) {
        uint256 byte_count = b.length;
        bytes memory flat_b = new bytes(byte_count);
        for (uint256 i = 0; i < byte_count; i++) {
    ...
    function str_cmp(string memory self, string memory other) internal pure returns (bool) {
